...
// download .sh button, its for actionsToSH()
let downloadSH = document.createElement('button');
downloadSH.innerHTML = 'Download .sh';
downloadSH.addEventListener('click', (e) => {
    let string = actionsToSH();
    let blob = new Blob([string], { type: 'text/plain' });
    let url = URL.createObjectURL(blob);
    let link = document.createElement('a');
    link.href = url;
    link.download = 'untitled.sh';
    link.click();
    URL.revokeObjectURL(url);
});
settingsArea.appendChild(downloadSH);
...
// .sh bash
function actionsToSH() {
    let result = '#!/bin/bash\n\n# textanim .sh\n# Generated by textanim webapp\n\n';
    result += 'clear\n\n';
    // hide cursor
    result += 'echo -e "\\033[?25l"\n';
    for (let action of actions) {
        if (action.type === 'frame') {
            let frame = action as Frame;
            //let text = frame.text.replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/`/g, '\\`');
            // split every cols
            let texts: string[] = [];
            let length = frame.text.length;
            for (let i = 0; i < length; i += cols) {
                texts.push('▕' + frame.text.slice(i, i + cols) + '▏');
            }
            texts = texts.map((text) => {
                return text.replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/`/g, '\\`');
            });
            result += 'echo -e " ' + '▁'.repeat(cols) + '\\n' + texts.join('\\n') + '\\n ' + '▔'.repeat(cols) + '\\n untitled.textanim (sh)\\n"\n';
            result += 'sleep ' + (frameDelay / 1000) + '\n';
            // move cursor to top left
            result += 'echo -e "\\033[H"\n';
        }
        else if (action.type === 'delay') {
            result += 'sleep ' + (frameDelay * (action as Delay).frames / 1000) + '\n';
            // move cursor to top left
            result += 'echo -e "\\033[H"\n';
        }
    }
    // show cursor and clear
    result += 'echo -e "\\033[?25h"\n\nclear\n';
    return result;
}
// @ts-ignore
window.actionsToSH = actionsToSH;
