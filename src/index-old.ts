...
function renderActions() {
    ...
    for (let i = 0; i < actionLength; i++) {
        ...
        if (action.type === 'frame') {
            ...
            frameReplica.style.fontSize = (frameReplica.rows) + 'ch';
            ...
            frameReplica.addEventListener('mouseover', (e) => {
                if (i === editingFrameIndex) {
                    ...
                    previewTextArea.style.fontSize = (previewTextArea.rows) + 'ch';
...
textArea.addEventListener('keydown', (e) => {
    // if our selection isnt both at same spot, set both to end
    ...
    // if its newline, we will instead jump to next line without inserting newline, no content will be modified
        ...
        let selectionEnd = textArea.selectionEnd + colLength;
        selectionEnd -= selectionEnd % colLength;
        textArea.selectionStart = selectionEnd;
        textArea.selectionEnd = selectionEnd;
        e.preventDefault();
    }
    // if about to insert a char, replace instead
        ...
        let selectionEnd = textArea.selectionEnd;
        textArea.value = textArea.value.slice(0, textArea.selectionEnd) + e.key + textArea.value.slice(textArea.selectionEnd + 1);
        e.preventDefault();
    ...
    // backspace should just set blank
    else if (e.key === 'Backspace') {
        e.preventDefault();
        let selectionEnd = textArea.selectionEnd;
        // selectionEnd - 1 if possible, otherwise do nothing
        if (selectionEnd > 0) {
            // now lets set a space
    ...
    // delete key is same except other direction
    ...
    // custom handling of arrow keys since it behaves weirdly otherwise
...
addRow.addEventListener('click', () => {
    ...
    previewTextArea.rows = textArea.rows;
    previewTextArea.style.fontSize = (previewTextArea.rows) + 'ch';
    textArea.style.fontSize = (textArea.rows) + 'ch';
    prevTextArea.style.fontSize = (prevTextArea.rows) + 'ch';
    fixTextArea();
    renderActions();
});
...
let textArea2Div = document.createElement('div');
textArea2Div.className = 'textarea2';
textArea2Div = document.body.appendChild(textArea2Div);
let textArea2 = new TextArea(textArea2Div);
textArea2.value = 'hello world';
textArea2.cols = 16;
textArea2.rows = 3;
// @ts-ignore
window.textArea2 = textArea2;